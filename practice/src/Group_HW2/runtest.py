...
class TestTool:
    ...
    # Comparing expected result to actual result (line by line)
    ...
    def run(self, cmd):
        ...
        for case in cmd['cases']:
            ...
            if self.check(expected, actual) and self.check(expected_err, actual_err):
...
if __name__ == "__main__":
   if len(sys.argv) < 2:
        ...
        exit(1)
   x = TestTool()
   testcase = x.loadTest(sys.argv[1])
   #testcase = x.loadTest("MultipleLine.json")
   x.build(testcase)
   print(x.run(testcase))